...
import addNewStreamer from "./NewStreamer";
...
const TwitchRandomStreamer = () => {
  ...
  useEffect(() => {
    const fetchToken = async () => {
      try {
        ...
        setError((prev) =>
          prev
            ? prev + " | Failed to fetch access token."
  ...
  const fetchRandomStreamer = async () => {
    ...
    try {
      ...
      setError((prev) =>
        prev
          ? prev + " | Failed to fetch streamers."
  ...
  const fetchStreamerVideos = async (userId) => {
    ...
    try {
      ...
      if (response.data.data.length > 0) {
        ...
        setError((prev) =>
          prev ? prev + " | No videos found." : "No videos found."
        );
      }
    } catch (error) {
      console.error("Error fetching videos:", error);
      setError((prev) =>
        prev ? prev + " | Failed to fetch videos." : "Failed to fetch videos."
      );
    }
  };
